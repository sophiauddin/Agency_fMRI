...
I used the built-in batch editor GUI to add each of the 3 trial types (lev, self, and dic) as separate conditions to the first level model.
...
The design matrix now looks like this (example=subject 1):
...
I did the second-level analysis both with and without including block order as another regressor. **Examine how block order changes actual p values**
